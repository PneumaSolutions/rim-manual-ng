...
A ativação do botão "Organization" (Organização) no painel do RIM revelará um menu de opções relacionadas à organização e ao gerenciamento de membros. A página "Members" (Membros) listará todos os membros de sua organização. Todos os membros podem ter sua função e nome modificados, e qualquer membro pode ser removido de uma organização, exceto o proprietário da organização. Adicionar membros é tão simples quanto inserir o endereço de e-mail, selecionar a função (usuário ou administrador) e, opcionalmente, inserir o nome.  
Se você tiver vários membros para adicionar, poderá usar o campo de edição "Members" para fazer uma adição em massa. Esse campo de edição aceita várias linhas de texto, permitindo que você insira as informações de cada membro em uma linha separada. Em cada linha, você pode inserir um único endereço de e-mail ou um endereço de e-mail e um nome no formato:  
...
Todo o seu histórico de sessões pode ser exportado para visualização posterior por meio de um aplicativo de planilha eletrônica. Ative o botão "Download session history as CSV" (Baixar histórico da sessão como CSV) localizado abaixo da tabela de histórico da sessão para revelar uma página de personalização para a exportação de dados. Nessa página, você pode escolher o período de tempo da exportação, bem como o símbolo usado para separar os campos.

### Gerenciando sua assinatura (planos Personal e Pro)

O painel permite que você visualize e gerencie facilmente os detalhes da sua assinatura da RIM. Ao clicar no link "gerenciar assinatura", você pode:
