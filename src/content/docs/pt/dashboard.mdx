...
Dependendo da natureza da sua organização, pode haver determinados recursos aos quais você não deseja que a sua equipe tenha acesso. Dessa forma, os administradores da organização podem escolher os recursos aos quais desejam permitir o acesso dos controladores. A tela "Restrictions" (Restrições), acessada pelo menu "Organization" (Organização), permite configurar essas permissões.

As permissões que podem ser alternadas incluem:
* Permitir que o controlador vire a sessão
    * Se essa opção estiver desmarcada, a opção "Flip Session" desaparecerá do menu da sessão e o atalho do teclado não funcionará.
* Permitir que o controlador inicie uma conversa por voz
    * Se essa opção estiver desmarcada, a caixa de seleção "Start Voice Conversation" (Iniciar conversa por voz) desaparecerá da tela "Provide Remote Help" (Fornecer ajuda remota), assim como a opção de menu no menu da sessão.
* Permitir que o controlador solicite acesso sem supervisão
    * Se essa opção estiver desmarcada, a opção de solicitar acesso sem supervisão desaparecerá do menu da sessão.
* Permitir que texto e arquivos sejam transferidos entre máquinas por meio da área de transferência
...
* Permitir que o controlador bloqueie a máquina de destino
    * Se essa opção estiver desmarcada, a opção de bloquear a máquina de destino desaparecerá do menu da sessão.

Depois de configurar as permissões de forma satisfatória, ative o botão "Update" (Atualizar). As alterações não afetarão nenhuma sessão ativa no momento em que as permissões forem alteradas. No entanto, elas entrarão em vigor para toda e qualquer sessão futura sem exigir a reinicialização do cliente.
...
1. Você será perguntado por quanto tempo deseja que esse instalador seja válido. Ele pode ser válido por qualquer período entre 7 e 30 dias ou permanentemente válido para planos empresariais, definindo a expiração como nunca.
...
1. Em seguida, será apresentada uma caixa de seleção que permite que você opte por receber uma notificação por e-mail quando um usuário instalar a RIM por meio desse instalador.
...
1. Clique em "Build Installer" (Criar instalador) Será apresentado o link de download que pode ser copiado para a área de transferência e enviado ao usuário final. Como alternativa, você pode fazer o download do instalador diretamente para uso em implementações em massa. Você pode excluir o instalador dessa tela se optar por torná-lo inoperante prematuramente.

Agora que você tem o instalador, ele pode ser executado de duas maneiras. Em ambos os casos, o instalador consultará o servidor para verificar a validade do instalador. Se o instalador for válido, a máquina será adicionada à sua lista de máquinas em sua conta e no cliente RIM após a conclusão do instalador. Um instalador inválido não será executado.

#### Execução normal

O usuário receberá um aviso ao executar o instalador, contendo as seguintes informações:
* O nome do técnico e sua organização, se aplicável
