...
* Implementa una finestra di dialogo di consenso sul lato destinatario prima dell&#x27;inizio della registrazione di una sessione durante qualsiasi sessione basata su parole chiave. Se l&#x27;utente destinatario rifiuta la richiesta di registrazione, la sessione termina.
...
### 3.4.7
Questa versione introduce una beta aperta della funzionalità di registrazione delle sessioni su entrambe le piattaforme. Questa funzionalità è gratuita per tutti e il servizio cloud non viene mai utilizzato per l&#x27;elaborazione delle registrazioni. Le registrazioni vengono attualmente salvate nella cartella Video locale dell&#x27;utente che esegue il controllo, ovvero &quot;Video&quot; su Windows e &quot;Film&quot; su MacOS.  
Non esitate a [inviarci un feedback](https://getrim.app/feedback) se riscontrate problemi durante l&#x27;utilizzo di questa funzione o se avete commenti o suggerimenti su come migliorarla.
### 3.4.6
...
* macOS: RIM è ora compatibile con le versioni recenti dello strumento di mappatura della tastiera Karabiner Elements.

### 3.4.3

* Windows: La funzionalità di accessibilità remota è ora compatibile con NVDA 2025.1.

### 3.4.2

* macOS: quando si inverte una sessione, RIM ora interrompe l'accessibilità remota se tale funzione era attiva sul precedente obiettivo.

### 3.4.1

* Windows: Quando si esegue un programma di installazione di destinazione preconfigurato in modalità silenziosa, RIM ora registra in modo più affidabile il computer di destinazione e inizia ad accettare connessioni non presidiate.

### 3.4.0
* Aggiunta un'opzione per gli utenti aziendali per la creazione di programmi di installazione senza scadenza
	* Questa modifica comporta un controllo di convalida per tutti gli installatori personalizzati. Se un programma di installazione personalizzato è scaduto o eliminato, non verrà più eseguito.
* È stato risolto un problema che causava un bip costante dai computer di destinazione privi di audio a bordo, in particolare i server Windows
* Non è più possibile premere involontariamente Windows+Maiusc+Backspace su una destinazione Windows con macOS come controller

### 3.3.39

* macOS: aggiunta una nota sulle autorizzazioni necessarie alla schermata di benvenuto.

### 3.3.38

* È stato risolto un altro problema che causava l'imprecisione della mappatura tra i cursori del controller e del mouse di destinazione. Fateci sapere se continuate ad avere problemi in quest'area.

### 3.3.37

* È stato risolto un problema che causava l'imprecisione della mappatura tra i cursori del controller e del mouse di destinazione.

### 3.3.36

* Migliorata l'affidabilità del supporto di RIM per la connessione attraverso firewall rigidi.
...
* Windows: È stato risolto un problema che causava l'invisibilità del cursore del mouse nella finestra della sessione remota del controller quando ci si connetteva ad alcuni target Windows senza mouse collegato.

### 3.3.33

* macOS: RIM ha ora ottenuto da Apple l'autorizzazione necessaria per consentire l'accesso remoto, compreso l'accesso non presidiato, senza richiedere nuovamente l'autorizzazione alla registrazione dello schermo ogni mese. Ciò richiede che gli utenti target che eseguono macOS Sequoia concedano una nuova autorizzazione di "accesso remoto" solo una volta. RIM richiederà questa autorizzazione prima di visualizzare la schermata di ricezione dell'aiuto o al primo avvio dopo l'installazione dell'aggiornamento sui computer registrati come target non presidiati. Si noti che c'è ancora un problema noto con RIM su macOS Sequoia: la funzione di accessibilità remota sui computer target di macOS Sequoia non funziona correttamente. Stiamo attualmente collaborando con Apple per risolvere questo problema. Se voi o i vostri utenti target siete interessati da questo problema, continuiamo a consigliarvi di evitare l'aggiornamento a macOS Sequoia fino a nuovo avviso. Grazie per la pazienza.

### 3.3.32

* È stato risolto un problema che a volte causava l'arresto anomalo di RIM durante la copia di una cartella negli appunti.
* È stato risolto un problema che a volte causava il blocco e la riconnessione della sessione quando si copiava una cartella di grandi dimensioni negli appunti.
* Windows: Corretto un problema che a intermittenza faceva sì che RIM non notasse quando un nuovo file o un insieme di file veniva copiato negli appunti.

### 3.3.31

* macOS: Aggiornate le istruzioni per la concessione delle autorizzazioni necessarie su macOS Sequoia. Si noti che ci sono ancora due problemi noti con RIM su macOS Sequoia: macOS richiede nuovamente all'utente di destinazione l'autorizzazione alla registrazione dello schermo ogni mese, interferendo con le sessioni non presidiate, e la funzione di accessibilità remota sui computer di destinazione di macOS Sequoia non funziona correttamente. Stiamo attualmente collaborando con Apple per risolvere questi problemi. Se voi o i vostri utenti target siete interessati da uno di questi problemi, continuiamo a consigliarvi di evitare l'aggiornamento a macOS Sequoia fino a nuovo avviso. Grazie per la vostra pazienza.

### 3.3.30

* È stato risolto un problema che impediva alla funzione Invia registri di funzionare in modo affidabile.
...
Questo aggiornamento consente di installare un profilo server aggiuntivo. Ciò è utile per i destinatari del supporto tecnico che potrebbero ricevere assistenza da un'organizzazione che utilizza un'implementazione RIM privata rispetto al nostro cloud pubblico, nonché per i controllori che potrebbero lavorare con più di un server alla volta.

### 3.3.27

* Questo aggiornamento introduce alcune modifiche al modo in cui RIM visualizza il puntatore del mouse e gestisce l'input del mouse. Risolve alcune situazioni in cui RIM mostrava due puntatori del mouse nell'area del contenuto della finestra Sessione remota, nonché un caso noto in cui non veniva mostrato alcun puntatore in quell'area, in particolare quando nessun mouse era collegato al computer di destinazione. Quando ci si connette agli obiettivi Windows, RIM è ora spesso in grado di utilizzare le impostazioni del puntatore del mouse del controller all'interno della finestra della sessione remota, rendendola più accessibile agli utenti ipovedenti. Inoltre, la conversione dell'input del mouse dal display del controller a quello di destinazione è ora più accurata.

### 3.3.24

* La finestra Sessione remota può ora essere spostata e ridimensionata, sia all'interno di un singolo display che su più display. La barra del titolo di questa finestra presenta ora anche i controlli standard della piattaforma per chiudere la finestra, massimizzarla e così via. Si noti che l'icona RIM, su cui si può fare clic per aprire il menu, si trova ora al centro della barra del titolo e non più all'estremità sinistra.
* Windows: Corretto un bug che causava l'arresto anomalo di RIM durante la copia del testo negli appunti in Adobe Reader.

### 3.3.23

* Il controller può ora disattivare l'uscita audio della destinazione. A tale scopo, selezionare la casella di controllo "Mute target audio output" quando si avvia una sessione basata su parole chiave, oppure scegliere "Mute Target Audio Output" dal menu della sessione.

### 3.3.22

* Nella finestra Fornisci aiuto, mentre si attende che l'utente di destinazione immetta la parola chiave, RIM ora visualizza la parola chiave immessa e un conto alla rovescia fino alla scadenza della parola chiave. È presente anche un pulsante "Annulla", che consente di annullare facilmente il tentativo in corso senza chiudere completamente la finestra; ciò è particolarmente comodo se si desidera provare una parola chiave diversa.
* Gli abbonati a RIM Enterprise e RIM Pro con il componente aggiuntivo Enterprise possono ora limitare alcune funzioni RIM all'interno dell'organizzazione. Attualmente è possibile limitare le seguenti funzioni: capovolgimento delle sessioni, conversazioni vocali, richiesta di accesso non presidiato da parte dei controller, trasferimento di testo e file tra i computer tramite gli appunti e blocco del computer di destinazione da parte dei controller. Gli amministratori dell'organizzazione possono accedere a queste impostazioni tramite la voce "Restrizioni" nel menu a discesa "Organizzazione" della RIM Dashboard.

### 3.3.21

* È stato risolto un altro problema che causava la perdita intermittente della connettività dell'applicazione RIM.

### 3.3.20

* È stato risolto un problema che a volte causava la disconnessione di un computer su cui è in esecuzione RIM dal servizio RIM e non la riconnessione automatica, richiedendo il riavvio di RIM.

### 3.3.19

* Quando si trasferiscono i file utilizzando l'opzione nel menu RIM, RIM ora mostra la percentuale di completamento nella barra del titolo della finestra di avanzamento.
* Windows: RIM utilizza ora un metodo più robusto per passare dalla schermata di accesso a Windows al desktop principale. Questo dovrebbe essere particolarmente utile sui PC di destinazione in cui l'utente non ha la password di Windows.
* macOS: nelle nuove installazioni, RIM ora chiede le autorizzazioni dopo la schermata di benvenuto e le autorizzazioni richieste dipendono dalla decisione di ricevere o fornire aiuto.

### 3.3.18

* Windows: Corretto un problema che causava il blocco di RIM se la sessione terminava o si riconnetteva durante un trasferimento di file.

### 3.3.17

* Windows: Risolto un problema che faceva sì che il programma di installazione di RIM per Windows venisse segnalato come scaricato in modo insolito e potenzialmente non sicuro da Microsoft SmartScreen.
...
installazioni, questo sostituisce la richiesta di autorizzazione per il monitoraggio dell'input, per cui
...
mac. Utilizzando una speciale configurazione di VoiceOver sul computer di destinazione, il controllore, sia esso Windows o Mac, potrà
