...
Va notato che le funzionalità del dashboard dipendono in larga misura dal livello di abbonamento dell'account. Ad esempio, gli utenti aziendali possono assegnare i computer a gruppi di destinazione e creare installatori silenziosi. D'altro canto, chiunque (compresi gli utenti privati) può creare installatori personalizzati per i propri computer non presidiati.
...
1. Verrà chiesto per quanto tempo si desidera che il programma di installazione sia valido. La validità può essere compresa tra i 7 e i 30 giorni, oppure può essere permanente per i piani aziendali impostando la scadenza su mai.
1. Viene quindi data la possibilità di assegnare un nome al basso. A qualsiasi macchina approvvigionata tramite questo pacchetto di installazione verrà assegnato questo nome di base.
...
1. Fare clic su "Build Installer" Verrà visualizzato il link per il download che può essere copiato negli appunti e inviato all'utente finale. In alternativa, è possibile scaricare direttamente il programma di installazione per utilizzarlo nelle distribuzioni di massa. È possibile eliminare il programma di installazione da questa schermata se si decide di renderlo prematuramente inutilizzabile.

Una volta ottenuto il programma di installazione, è possibile eseguirlo in due modi. In entrambi i casi, il programma di installazione interroga il server per verificare la validità del programma di installazione. Se il programma di installazione è valido, il computer verrà aggiunto all'elenco dei computer dell'account e del client RIM al termine dell'installazione. Un programma di installazione non valido non verrà eseguito.
