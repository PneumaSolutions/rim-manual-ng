...
<MacOS>
A questo punto si è pronti per eseguire qualsiasi operazione sul lato client. Se si desidera tornare a controllare il proprio computer, richiamare il menu RIM e selezionare l'opzione "Riduci a icona la sessione". Si tornerà al proprio computer fino a quando non si passerà nuovamente alla finestra di sessione o non si premerà nuovamente Opzione+Maiusc+Backspace. Quando si torna alla finestra di sessione, gli input della tastiera e del mouse vengono nuovamente indirizzati al computer client.
</MacOS>
...
<MacOS>
- Aggiorna RIM sul target: Forza l'aggiornamento di RIM sul computer di destinazione
    - Questa opzione viene visualizzata se sul computer di destinazione è in esecuzione una versione precedente di RIM.
- Reinstalla RIM come amministratore sul target: Questa opzione tenta di reinstallare RIM come amministratore su un computer di destinazione Windows, richiedendo all'utente di destinazione di accettare la richiesta di autorizzazione di amministratore da solo e/o con l'aiuto dell'IT aziendale.
- Riduci a icona la sessione (**Opzione+Maiusc+M**): riporta il controllo al computer dell'utente come descritto in precedenza
- Trasferisci file a destinazione: Appare se un file è presente negli appunti.
    - Sebbene questo metodo alternativo per completare un trasferimento di file sia sempre disponibile, è necessario solo per trasferire file a destinazioni Mac.
...
<MacOS>
Il modulo di accessibilità remota è una configurazione speciale avviata sul Mac di destinazione su richiesta di un controller Windows o Mac. Il vantaggio di questo approccio è che l'utente finale non sente la voce del computer mentre lo si controlla. Al contrario, il modulo di accessibilità remota trasmette l'output vocale al sistema vocale del controllore. In questo modo, è possibile assistere l'utente finale in modo accessibile senza che debba attivare uno screen reader. Se sul Mac di controllo è in esecuzione VoiceOver, il modulo di accessibilità remota si avvia automaticamente durante le sessioni remote in cui il computer remoto non ha un lettore di schermo in esecuzione.
</MacOS>
...
1. Verrà chiesto di assegnare un nome al computer. Inserire un nome personale per il computer o, se applicabile, l'ID del computer come appare nel gruppo di lavoro.
1. Premere Invio.
...
Le scelte rapide di sessione non presidiate, come qualsiasi altra scelta rapida, possono essere associate a tasti di scelta rapida globali. Questo può essere estremamente utile se si è un tecnico della manutenzione che gestisce più computer in un gruppo di lavoro. Ad esempio, se il vostro gruppo di lavoro è composto da 6 computer sui quali eseguite la manutenzione ordinaria, potete configurare Alt+Control+1 fino a 6 come tasti di scelta rapida per ogni rispettivo computer. Questo dovrebbe accelerare notevolmente il flusso di lavoro.
...
È tutto! Il controllore riceverà un messaggio che indica che la macchina non è più disponibile per l'accesso non presidiato. Se dovesse avere nuovamente bisogno di un accesso non presidiato, può avviare nuovamente la procedura di richiesta di autorizzazione all'accesso non presidiato come descritto in precedenza.
<Windows>
riferimento al comando chiave ###

| Azione | Comando |
| ---------------------------------------- | ------------------- |
| Accesso al menu RIM | Windows+Shift+Backspace |
| Riduci a icona la sessione | Windows+Shift+M |
| Ribalta la sessione | Windows+Shift+F |
| Disconnetti la sessione (controller o target) | Windows+Shift+D |
</Windows>
<MacOS>
### Riferimento ai comandi chiave
