...
### 3.4.7
This build introduces the session recording capability across both platforms. This capability is free for anyone to use, and the cloud service is never utilized for processing the recording. Recordings are currently saved to the controlling user's local Videos folder, that being "Videos" on Windows and "Movies" on MacOS respectively.
### 3.4.6
...
* macOS: RIM is now compatible with recent versions of the Karabiner Elements keyboard mapping tool.

### 3.4.3

* Windows: The remote accessibility feature is now compatible with NVDA 2025.1.

### 3.4.2

* macOS: When flipping a session, RIM now stops remote accessibility if that feature was active on the former target.

### 3.4.1

* Windows: When running a pre-configured target installer in silent mode, RIM now more reliably registers the target machine and starts accepting unattended connections.
...
* macOS: Added a note about the required permissions to the welcome screen.
...
* Windows: Fixed a bug that caused RIM to crash when copying text to the clipboard in Adobe Reader.
...
* Windows: RIM now uses a more robust method of transitioning from the Windows Logon screen to the main desktop. This should especially help on target PCs where the user has no Windows password.
* macOS: In new installations, RIM now asks for permissions after the welcome screen, and the requested permissions depend on whether you decide to receive or provide help.

### 3.3.18

* Windows: Fixed a problem that caused RIM to lock up if the session ended or reconnected during a file transfer.

### 3.3.17

* Windows: Solved a problem that caused the RIM Windows installer to be flagged as uncommonly downloaded and potentially unsafe by Microsoft SmartScreen.
...
* macOS: The previous version for this platform didn't actually include the localization fixes.
