...
Activating the "Organization" button on the RIM dashboard will reveal a menu of options related to organization and member management. The "Members" page will list all the members in your organization. All members can have their role and name modified, and any member can be removed from an organization except for the organization owner. Adding members is as simple as entering their email address, selecting their role (user or administrator), and optionally entering their name.  
...
Your entire session history can be downloaded as a csv dump for later viewing via a spreadsheet application.
