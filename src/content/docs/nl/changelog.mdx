...
* Implementeert het vastleggen van de module voor externe toegankelijkheid tijdens een sessie-opname via het audiosysteem van de besturingscomputer.
* Implementeert een dialoogvenster voor toestemming aan de kant van de doelgroep voordat een sessieopname wordt gestart tijdens sessies op basis van trefwoorden. Als de doelgebruiker het verzoek om opname weigert, wordt de sessie beëindigd.
...
### 3.4.7
Deze build introduceert een open bèta van de sessie-opnamefunctie op beide platforms. Deze functie is gratis voor iedereen en de cloudservice wordt nooit gebruikt voor het verwerken van de opname. Opnames worden momenteel opgeslagen in de lokale map Video&#x27;s van de controlerende gebruiker, namelijk &quot;Video&#x27;s&quot; op Windows en &quot;Films&quot; op MacOS.  
Aarzel niet om [ons feedback te sturen](https://getrim.app/feedback) als u problemen ondervindt bij het gebruik van deze functie, of als u opmerkingen of suggesties heeft over hoe deze kan worden verbeterd.
### 3.4.6
...
* macOS: RIM is nu compatibel met recente versies van de Karabiner Elements keyboard mapping tool.

### 3.4.3

* Windows: De functie Toegankelijkheid op afstand is nu compatibel met NVDA 2025.1.

### 3.4.2

* macOS: wanneer een sessie wordt omgedraaid, stopt RIM nu externe toegankelijkheid als die functie actief was op het vorige doel.

### 3.4.1

* Windows: Wanneer een vooraf geconfigureerd doel-installatieprogramma in stille modus wordt uitgevoerd, registreert RIM nu betrouwbaarder de doelmachine en begint zonder toezicht verbindingen te accepteren.

### 3.4.0
* Een optie toegevoegd voor enterprise gebruikers om nooit-verlopende installatieprogramma's te maken
    * Met deze wijziging komt er een validatiecontrole voor alle aangepaste installatieprogramma's. Als een aangepast installatieprogramma verlopen of verwijderd is, wordt het niet meer uitgevoerd.
* Probleem opgelost dat een constante piep veroorzaakte op doelmachines zonder ingebouwde audio, met name Windows-servers
* Het is niet langer mogelijk om ongewild op Windows+Shift+Backspace te drukken op een Windows-doel met macOS als controller

### 3.3.39

* macOS: een opmerking over de vereiste machtigingen toegevoegd aan het welkomstscherm.

### 3.3.38

* Een ander probleem dat ervoor zorgde dat de muiscursors van de controller en het doel onnauwkeurig waren, is opgelost. Laat het ons weten als je problemen blijft ondervinden op dit gebied.

### 3.3.37

* Probleem opgelost waarbij de toewijzing tussen controller en doelmuiscursors onnauwkeurig was.

### 3.3.36

* Verbeterde betrouwbaarheid van RIM's ondersteuning voor verbinding door strikte firewalls.
...
* Windows: Probleem verholpen waarbij de muiscursor onzichtbaar was in het venster Remote Session van de controller bij verbinding met sommige Windows-doelen zonder muisverbinding.

### 3.3.33

* macOS: RIM heeft nu het benodigde recht van Apple om toegang op afstand toe te staan, inclusief toegang zonder toezicht, zonder elke maand opnieuw te vragen om de toestemming voor schermopname. Hiervoor hoeven doelgebruikers met macOS Sequoia slechts één keer een nieuwe toestemming voor "toegang op afstand" te geven. RIM zal om deze toestemming vragen voordat het het scherm Help ontvangen weergeeft, of de eerste keer dat het opstart na het installeren van deze update op machines die zijn geregistreerd als onbeheerde doelen. Er is nog steeds één bekend probleem met RIM op macOS Sequoia: de functie voor externe toegankelijkheid op macOS Sequoia doelmachines werkt niet correct. We werken momenteel samen met Apple om dit probleem op te lossen. Als jij of je doelgebruikers last hebben van dit probleem, raden we je aan om tot nader order geen upgrade naar macOS Sequoia uit te voeren. Hartelijk dank voor uw geduld.

### 3.3.32

* Probleem verholpen waarbij RIM soms crashte bij het kopiëren van een map naar het klembord.
* Probleem verholpen dat er soms voor zorgde dat de sessie bevroor en er opnieuw verbinding werd gemaakt bij het kopiëren van een grote map naar het klembord.
* Windows: Probleem verholpen dat er soms voor zorgde dat RIM niet opmerkte wanneer een nieuw bestand of een set bestanden naar het klembord werd gekopieerd.

### 3.3.31

* macOS: instructies bijgewerkt voor het toekennen van de benodigde machtigingen op macOS Sequoia. Er zijn nog twee bekende problemen met RIM op macOS Sequoia: macOS vraagt de doelgebruiker elke maand opnieuw om toestemming voor schermopname, waardoor sessies zonder toezicht worden verstoord, en de functie voor externe toegankelijkheid op macOS Sequoia-doelmachines werkt niet correct. We werken momenteel samen met Apple om deze problemen op te lossen. Als jij of je gebruikers last hebben van deze problemen, raden we je aan om tot nader order niet te upgraden naar macOS Sequoia. Hartelijk dank voor uw geduld.
...
* Deze update introduceert interne wijzigingen in het software-updateprogramma van RIM om het robuuster te maken.

### 3.3.28

Deze update maakt het mogelijk om een extra serverprofiel te installeren. Dit is handig voor ontvangers van technische ondersteuning die mogelijk ondersteuning krijgen van een organisatie die een private RIM-implementatie gebruikt in plaats van onze publieke cloud, en voor controllers die mogelijk met meer dan één server tegelijk werken.

### 3.3.27

* Deze update introduceert enkele wijzigingen in de manier waarop RIM de muisaanwijzer weergeeft en omgaat met muisinvoer. Hiermee worden enkele situaties opgelost waarin RIM twee muisaanwijzers weergaf in het inhoudsgebied van het Remote Session-venster, evenals een bekend geval waarin geen muisaanwijzer werd weergegeven in dat gebied, met name wanneer er geen muis was aangesloten op de doelcomputer. Bij het verbinden met Windows-doelen kan RIM nu vaak de muisaanwijzerinstellingen van de controller gebruiken in het Remote Session-venster, waardoor het toegankelijker wordt voor gebruikers met een visuele beperking. Ook is de conversie van muisinvoer van het scherm van de controller naar het doeldisplay nu nauwkeuriger.

### 3.3.24

* Het venster Remote Session kan nu worden verplaatst en van grootte worden veranderd, binnen één beeldscherm of over beeldschermen heen. De titelbalk voor dit venster heeft nu ook de platform-standaard besturingselementen om het venster te sluiten, te maximaliseren, enzovoort. Merk op dat het RIM pictogram, waarop kan worden geklikt om het menu te openen, nu in het midden van de titelbalk staat en niet meer aan de linkerkant.
* Windows: Bug verholpen waardoor RIM vastliep bij het kopiëren van tekst naar het klembord in Adobe Reader.

### 3.3.23

* De controller kan nu de audio-uitvoer van het doel dempen. Om dit te doen, vink je het selectievakje "Audio-uitgang van doelobject dempen" aan bij het starten van een sessie op basis van een trefwoord, of kies je "Audio-uitgang van doelobject dempen" in het sessiemenu.

### 3.3.22

* In het venster Hulp bieden toont RIM nu, terwijl het wacht tot de doelgebruiker het trefwoord invoert, het trefwoord dat je hebt ingevoerd en een aftelling tot het trefwoord verloopt. Er is ook een knop "Annuleren", zodat je de huidige poging eenvoudig kunt annuleren zonder het venster helemaal te sluiten; dit is vooral handig als je een ander trefwoord wilt proberen.
* Abonnees van RIM Enterprise en RIM Pro met de Enterprise add-on kunnen nu bepaalde RIM-functies binnen de organisatie beperken. De volgende functies kunnen momenteel worden beperkt: sessies omdraaien, spraakgesprekken, controllers die onbeheerde toegang aanvragen, tekst en bestanden overbrengen tussen machines via het klembord en controllers die de doelmachine vergrendelen. Organisatiebeheerders hebben toegang tot deze instellingen via het item "Beperkingen" in het vervolgkeuzemenu "Organisatie" in het RIM Dashboard.

### 3.3.21

* Probleem verholpen waarbij de RIM-app af en toe de verbinding verbrak.

### 3.3.20

* Probleem opgelost dat er soms voor zorgde dat een machine waarop RIM werd uitgevoerd de verbinding met de RIM-service verbrak en niet automatisch opnieuw verbinding maakte, waardoor RIM opnieuw moest worden opgestart.

### 3.3.19

* Bij het overzetten van bestanden via de optie in het RIM-menu toont RIM nu het voltooide percentage in de titelbalk van het voortgangsvenster.
* Windows: RIM gebruikt nu een robuustere methode voor de overgang van het Windows aanmeldscherm naar het hoofdbureaublad. Dit zou vooral moeten helpen op doel-pc's waar de gebruiker geen Windows-wachtwoord heeft.
* macOS: In nieuwe installaties vraagt RIM nu om machtigingen na het welkomstscherm, en de gevraagde machtigingen zijn afhankelijk van of je besluit om hulp te ontvangen of te geven.

### 3.3.18

* Windows: Probleem verholpen waarbij RIM vastliep als de sessie werd beëindigd of opnieuw verbinding werd gemaakt tijdens een bestandsoverdracht.
