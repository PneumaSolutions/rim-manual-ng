...
Als u de knop "Organisatie" op het RIM dashboard activeert, verschijnt er een menu met opties voor organisatie- en ledenbeheer. De pagina "Leden" geeft een overzicht van alle leden in je organisatie. Alle leden kunnen hun rol en naam wijzigen en elk lid kan uit een organisatie worden verwijderd, behalve de eigenaar van de organisatie. Leden toevoegen is zo eenvoudig als het invoeren van hun e-mailadres, het selecteren van hun rol (gebruiker of beheerder) en optioneel het invoeren van hun naam.  
Als je meerdere leden wilt toevoegen, kun je het bewerkingsveld "Leden" gebruiken om in bulk toe te voegen. Dit bewerkingsveld accepteert meerdere regels tekst, zodat u de informatie van elk lid op een aparte regel kunt invoeren. Op elke regel kun je één e-mailadres invoeren of een e-mailadres en een naam in de indeling:  
...
1. Als je een enterprise admin bent, zie je een selectievakje waarmee je het installatieprogramma als een MSI-pakket kunt bouwen. Deze optie is handig voor massale implementatie van een aangepast installatieprogramma op een machinecluster dat wordt toegewezen aan de opgegeven doelgroep.
...
Je volledige sessiegeschiedenis kan worden gedownload als een csv-dump om later te bekijken via een spreadsheetapplicatie.
