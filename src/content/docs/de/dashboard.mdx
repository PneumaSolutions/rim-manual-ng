...
Wenn Sie die Schaltfläche "Organisation" auf dem RIM-Dashboard aktivieren, wird ein Menü mit Optionen für die Verwaltung von Organisationen und Mitgliedern angezeigt. Auf der Seite "Mitglieder" werden alle Mitglieder Ihrer Organisation aufgelistet. Alle Mitglieder können ihre Rolle und ihren Namen ändern, und jedes Mitglied kann aus einer Organisation entfernt werden, mit Ausnahme des Eigentümers der Organisation. Das Hinzufügen von Mitgliedern ist so einfach wie die Eingabe ihrer E-Mail-Adresse, die Auswahl ihrer Rolle (Benutzer oder Administrator) und optional die Eingabe ihres Namens.  
Wenn Sie mehrere Mitglieder hinzufügen möchten, können Sie das Bearbeitungsfeld "Mitglieder" verwenden, um eine Massenaufnahme durchzuführen. Dieses Feld kann mehrere Textzeilen aufnehmen, so dass Sie die Informationen zu jedem Mitglied in eine eigene Zeile eingeben können. In jeder Zeile können Sie entweder eine einzelne E-Mail-Adresse oder eine E-Mail-Adresse und einen Namen in dem Format eingeben:  
...
Wenn Ihr Unternehmen einen Supporttechniker einer bestimmten Gruppe von Rechnern zuweist, möchten Sie wahrscheinlich sicherstellen, dass er nur auf diese bestimmte Gruppe Zugriff hat. Hier kommt die Zugriffskontrolle für Zielgruppen ins Spiel.  
...
Ihr gesamter Sitzungsverlauf kann zur späteren Ansicht über eine Tabellenkalkulationsanwendung exportiert werden. Aktivieren Sie die Schaltfläche "Sitzungsverlauf als CSV herunterladen", die sich unter der Tabelle des Sitzungsverlaufs befindet, um eine Anpassungsseite für Ihren Datenexport zu öffnen. Auf dieser Seite können Sie die Zeitspanne für den Export sowie das Symbol zur Trennung der Felder auswählen.
