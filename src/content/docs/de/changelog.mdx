...
* Implementiert einen Dialog zur Einholung der Zustimmung des Zielbenutzers vor Beginn einer Sitzungsaufzeichnung während einer keywordbasierten Sitzung. Lehnt der Zielbenutzer die Aufzeichnung ab, wird die Sitzung beendet.
...
* Implementiert die Möglichkeit, das Standardverzeichnis für Aufzeichnungen sowie mehrere andere Konfigurationsparameter über eine TOML-Datei festzulegen. Anweisungen zum Konfigurieren dieser Optionen finden Sie im Abschnitt über Sitzungsaufzeichnungen.


### 3.4.7
Diese Version führt eine offene Beta-Version der Sitzungsaufzeichnungsfunktion auf beiden Plattformen ein. Diese Funktion kann von jedem kostenlos genutzt werden, und der Cloud-Dienst wird niemals für die Verarbeitung der Aufzeichnung verwendet. Die Aufzeichnungen werden derzeit im lokalen Videobereich des steuernden Benutzers gespeichert, d. h. unter „Videos” bei Windows und „Movies” bei MacOS.  
...
* macOS: RIM ist jetzt mit den neuesten Versionen des Tastaturbelegungsprogramms Karabiner Elements kompatibel.

### 3.4.3

* Windows: Die Fernzugriffsfunktion ist jetzt mit NVDA 2025.1 kompatibel.

### 3.4.2

* macOS: Wenn eine Sitzung gewechselt wird, beendet RIM jetzt den Fernzugriff, wenn diese Funktion auf dem früheren Ziel aktiviert war.

### 3.4.1

* Windows: Wenn ein vorkonfiguriertes Ziel-Installationsprogramm im Silent-Mode ausgeführt wird, registriert RIM jetzt zuverlässiger den Zielrechner und beginnt, unbeaufsichtigte Verbindungen zu akzeptieren.

### 3.4.0
* Es wurde eine Option für Unternehmensanwender hinzugefügt, um nie ablaufende Installationsprogramme zu erstellen
    * Mit dieser Änderung werden alle benutzerdefinierten Installer auf ihre Gültigkeit überprüft. Wenn ein benutzerdefinierter Installer abgelaufen ist oder gelöscht wurde, wird er nicht mehr ausgeführt.
* Es wurde ein Problem behoben, das ein ständiges Piepen von Zielcomputern ohne integriertes Audiosystem verursachte, insbesondere von Windows-Servern
* Es ist nicht mehr möglich, unwissentlich die Tastenkombination Windows+Umschalt+Rücktaste auf einem Windows-Zielrechner mit macOS als Controller zu drücken
