...
1. Tapez sndvol, puis appuyez sur la touche Entrée.
1. Appuyez sur la touche de fin pour atteindre 0 %, puis sur la flèche du haut pour atteindre 1 %. Comme vous pouvez le constater, RIM ne tient pas compte du volume réel de la machine cible lorsqu'il émet son son.
...
1. Appuyez à nouveau sur Windows+R et, cette fois, tapez mmsys.cpl pour ouvrir les paramètres sonores.
1. Une fois dans les paramètres sonores, accédez à l'onglet d'enregistrement (Ctrl+Tab).
1. Sélectionnez le microphone ou le périphérique d'entrée en ligne que vous souhaitez écouter, puis accédez aux propriétés avec Alt+P.
1. Une fois dans les propriétés, appuyez sur CTRL+Tab pour accéder à l'onglet d'écoute et cochez la case "Écouter ce périphérique".
1. Faites CTRL+Tab une fois de plus dans la section Niveaux et réglez le gain du microphone à un niveau qui vous conviendra. Appuyez sur la touche Entrée lorsque vous avez terminé.
