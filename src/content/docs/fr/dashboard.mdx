...
Il convient de noter que l'ensemble des fonctionnalités du tableau de bord dépend en grande partie de la catégorie d'abonnement à laquelle appartient votre compte. Par exemple, les utilisateurs professionnels peuvent affecter des machines à des groupes cibles et créer des programmes d'installation silencieux. D'autre part, tout le monde (y compris les utilisateurs personnels) peut créer des installateurs personnalisés pour leurs machines sans surveillance.
...
L'ensemble de l'historique des sessions peut être téléchargé sous la forme d'un fichier csv afin d'être visualisé ultérieurement à l'aide d'un tableur.
