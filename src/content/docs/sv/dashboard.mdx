...
1. Du kommer att bli tillfrågad hur länge du vill att installationsprogrammet ska vara giltigt. Det kan vara giltigt mellan 7 och 30 dagar, eller permanent giltigt för företagsplaner genom att ställa in utgångsdatumet till aldrig.
1. Du får sedan möjlighet att tilldela ett basnamn. Alla maskiner som provisioneras via det här installationspaketet kommer att tilldelas det här basnamnet.
1. Du kommer sedan att presenteras med en kryssruta som låter dig välja ett e-postmeddelande när en användare installerar RIM via det här installationsprogrammet.
1. Om du är företagsadministratör kommer du att se en kryssruta som gör att du kan bygga installationsprogrammet som ett MSI-paket. Detta alternativ är användbart för massdistribution av ett anpassat installationsprogram till ett maskinkluster som kommer att utses till den givna målgruppen.
...
1. Klicka på "Build Installer" Du kommer att få en nedladdningslänk som du antingen kan kopiera till urklipp och skicka till slutanvändaren. Alternativt kan du ladda ner installationsprogrammet direkt för att använda det vid massdistributioner. Du kan ta bort installationsprogrammet från den här skärmen om du väljer att göra det oanvändbart i förtid.

Nu när du har ditt installationsprogram kan det köras på ett av två sätt. I båda fallen kommer installationsprogrammet att fråga servern för att kontrollera installationsprogrammets giltighet. Om installationsprogrammet är giltigt kommer maskinen att läggas till i listan över maskiner i både ditt konto och RIM-klienten efter att installationsprogrammet har slutförts. Ett ogiltigt installationsprogram kommer inte att köras.

#### Normal exekvering

Användaren får en prompt när installationsprogrammet körs, som innehåller följande information:
* Teknikerns namn och organisation, om tillämpligt
