...
* Implementerar inspelning av fjärråtkomstmodulen under en sessionsinspelning via systemljudet på den styrande datorn.
* Implementerar en dialogruta för samtycke från målsidan innan en sessionsinspelning startar under alla nyckelordsbaserade sessioner. Om mål användaren nekar begäran om inspelning avslutas sessionen.
...
### 3.4.7
Denna version introducerar en öppen betaversion av funktionen för sessionsinspelning på båda plattformarna. Funktionen är gratis för alla att använda, och molntjänsten används aldrig för att bearbeta inspelningen. Inspelningarna sparas för närvarande i den kontrollerande användarens lokala videomapp, det vill säga &quot;Videos&quot; i Windows och &quot;Movies&quot; i MacOS.  
...
* macOS: RIM är nu kompatibelt med de senaste versionerna av tangentbordsmappningsverktyget Karabiner Elements.

### 3.4.3

* Windows: Funktionen för fjärrtillgänglighet är nu kompatibel med NVDA 2025.1.

### 3.4.2

* macOS: När en session vänds stoppar RIM nu fjärråtkomst om den funktionen var aktiv på det tidigare målet.

### 3.4.1

* Windows: När du kör en förkonfigurerad målinstallatör i tyst läge registrerar RIM nu målmaskinen på ett mer tillförlitligt sätt och börjar acceptera obevakade anslutningar.

### 3.4.0
* Lagt till ett alternativ för företagsanvändare att skapa installatörer som aldrig går ut
    * Med denna ändring följer en valideringskontroll av alla anpassade installatörer. Om en anpassad installatör är utgången eller raderad kommer den inte längre att köras.
* Åtgärdade ett problem som orsakade ett konstant pipande från målmaskiner utan inbyggt ljud, särskilt Windows-servrar
* Det är inte längre möjligt att oavsiktligt trycka på Windows+Shift+Backspace på ett Windows-mål med macOS som styrenhet

### 3.3.39

* macOS: En notering om vilka behörigheter som krävs har lagts till på välkomstskärmen.

### 3.3.38

* Åtgärdade ett annat problem som gjorde att mappningen mellan styrenhetens och målets muspekare var felaktig. Meddela oss om du fortsätter att ha problem på det här området.

### 3.3.37

* Åtgärdade ett problem som gjorde att mappningen mellan styrenhetens och målets muspekare var felaktig.

### 3.3.36

* Förbättrat tillförlitligheten i RIM:s stöd för anslutning genom strikta brandväggar.
...
* Windows: Åtgärdade ett problem som gjorde att muspekaren var osynlig i styrenhetens Remote Session-fönster vid anslutning till vissa Windows-mål utan ansluten mus.

### 3.3.33

* macOS: RIM har nu den nödvändiga behörigheten från Apple för att tillåta fjärråtkomst, inklusive obevakad åtkomst, utan att behöva be om tillstånd för skärminspelning varje månad. Detta kräver att målanvändare som kör macOS Sequoia beviljar en ny "fjärråtkomst"-behörighet bara en gång. RIM kommer att begära denna behörighet innan den visar skärmen Ta emot hjälp, eller första gången den startar efter att ha installerat denna uppdatering på maskiner som är registrerade som obevakade mål. Observera att det fortfarande finns ett känt problem med RIM på macOS Sequoia: funktionen för fjärråtkomst på macOS Sequoia-målmaskiner fungerar inte korrekt. Vi arbetar för närvarande med Apple för att lösa detta problem. Om du eller dina målanvändare skulle påverkas av det här problemet fortsätter vi att rekommendera att du undviker att uppgradera till macOS Sequoia tills vidare. Tack för ditt tålamod.

### 3.3.32

* Åtgärdat ett problem som ibland gjorde att RIM kraschade när en mapp kopierades till urklipp.
* Åtgärdade ett problem som ibland ledde till att sessionen frös och kopplades om när en stor mapp kopierades till urklipp.
* Windows: Åtgärdade ett problem som gjorde att RIM ibland inte märkte när en ny fil eller uppsättning filer kopierades till urklipp.

### 3.3.31

* macOS: Uppdaterade instruktioner för att bevilja nödvändiga behörigheter på macOS Sequoia. Observera att det fortfarande finns två kända problem med RIM på macOS Sequoia: macOS frågar målanvändaren på nytt efter skärminspelningsbehörighet varje månad, vilket stör obevakade sessioner, och funktionen för fjärråtkomst på macOS Sequoia-målmaskiner fungerar inte korrekt. Vi arbetar för närvarande med Apple för att lösa dessa problem. Om du eller dina målanvändare skulle påverkas av något av dessa problem fortsätter vi att rekommendera att du undviker att uppgradera till macOS Sequoia tills vidare. Tack för ditt tålamod.

### 3.3.30

* Åtgärdade ett problem som hindrade funktionen Skicka loggar från att fungera tillförlitligt.

### 3.3.29

* Den här uppdateringen introducerar interna ändringar i RIM:s programuppdaterare för att göra den mer robust.

### 3.3.28

Den här uppdateringen gör det möjligt att installera ytterligare en serverprofil. Detta är användbart för mottagare av teknisk support som kan få support från en organisation som använder en privat RIM-distribution i motsats till vårt offentliga moln, samt för controllers som kan arbeta med mer än en server åt gången.

### 3.3.27

* Denna uppdatering introducerar några ändringar i hur RIM återger muspekaren och hanterar musinmatning. Den åtgärdar vissa situationer där RIM visade två muspekare i innehållsområdet i fönstret Fjärrsession, samt ett känt fall där ingen pekare visades i det området, särskilt när ingen mus var ansluten till målmaskinen. Vid anslutning till Windows-mål kan RIM nu ofta använda styrenhetens inställningar för muspekare i fönstret Remote Session, vilket gör det mer tillgängligt för användare med nedsatt syn. Dessutom är konverteringen av musinmatning från styrenhetens display till målets display nu mer exakt.

### 3.3.24

* Fönstret Remote Session kan nu flyttas och ändras i storlek, både inom en skärm och mellan olika skärmar. Titelfältet för detta fönster har nu också de plattformsstandardiserade kontrollerna för att stänga fönstret, maximera det och så vidare. Observera att RIM-ikonen, som kan klickas på för att öppna menyn, nu finns i mitten av titelfältet, inte i den vänstra änden.
* Windows: Åtgärdat en bugg som gjorde att RIM kraschade när text kopierades till urklipp i Adobe Reader.

### 3.3.23

* Styrenheten kan nu stänga av ljudutgången från målet. Detta gör du genom att markera kryssrutan "Mute target audio output" när du startar en nyckelordsbaserad session eller genom att välja "Mute Target Audio Output" i sessionsmenyn.

### 3.3.22

* I fönstret Provide Help, medan du väntar på att målanvändaren ska skriva in nyckelordet, visar RIM nu det nyckelord du skrev in, samt en nedräkning tills nyckelordet löper ut. Det finns också en "Avbryt"-knapp, så att du enkelt kan avbryta det aktuella försöket utan att stänga fönstret helt; detta är särskilt praktiskt om du vill prova ett annat nyckelord.
* Abonnenter på RIM Enterprise och RIM Pro med Enterprise-tillägget kan nu begränsa vissa RIM-funktioner inom organisationen. Följande funktioner kan för närvarande begränsas: session flipping, röstsamtal, controllers som begär obevakad åtkomst, överföring av text och filer mellan maskiner via urklipp och controllers som låser målmaskinen. Organisationsadministratörer kan komma åt dessa inställningar via alternativet "Begränsningar" i rullgardinsmenyn "Organisation" i RIM Dashboard.

### 3.3.21

* Åtgärdade ett annat problem som gjorde att RIM-appen periodvis förlorade anslutningen.

### 3.3.20

* Åtgärdade ett problem som ibland orsakade att en maskin som kör RIM kopplades bort från RIM-tjänsten och inte automatiskt återanslöts, vilket krävde att RIM startades om.

### 3.3.19

* När du överför filer med hjälp av alternativet på RIM-menyn visar RIM nu den slutförda procentsatsen i titelfältet i förloppsfönstret.
* Windows: RIM använder nu en mer robust metod för att övergå från inloggningsskärmen i Windows till huvudskrivbordet. Detta bör särskilt vara till hjälp på måldatorer där användaren inte har något Windows-lösenord.
* macOS: I nya installationer ber RIM nu om behörigheter efter välkomstskärmen, och de begärda behörigheterna beror på om du bestämmer dig för att ta emot eller ge hjälp.

### 3.3.18

* Windows: Åtgärdade ett problem som gjorde att RIM låste sig om sessionen avslutades eller återanslöts under en filöverföring.

### 3.3.17

* Windows: Löste ett problem som gjorde att RIM:s Windows-installationsprogram flaggades som ovanligt nedladdat och potentiellt osäkert av Microsoft SmartScreen.
...
mål. Med hjälp av en särskild konfiguration av VoiceOver på
